...
//	获取对应地址和端口的信息
    ...
    // Find an address to connect to & connect， 找到一个地址用于连接
...
//	读取地址信息
    ...
    // Get connection, welcome messages，连接完毕，得到欢迎信息
    printf("Connected to %s.\n", host);
    print_reply(read_reply());


    /* Get name and password and send to server 从命令行中获取 账户名和密码用于连接 服务端 */
    ftclient_login();

    while (1)
    { // loop until user types quit

        // Get a command from user， 从命令行中获取 cmd 和其他参数
        ...
        // Send command to server 发送 存入 buffer 中的命令信息到服务端
        ...
        } else
        {
            ...
            // open data connection， 开启一个新的 socket 连接用于接收数据
